...
decay_parameter_tune <- function(constructor_span, driver_span, constructor_weight, driver_weight,
                                 max_history_value){
  ...
  position_pred_weights <- results_full %>%
    ...
    mutate(total_cars = max(position),
           ...
           position_pred_weight = ifelse(position_weighted,
  ...
  mae_unweighted_loess <- sum(results_pred_test$position_pred_diff_loess)/nrow(results_pred_test)
  mae_weighted_loess <- sum(results_pred_test$position_pred_diff_loess * results_pred_test$position_pred_weight)/sum(results_pred_test$position_pred_weight)
  ...
  return(list(
    ...
    mae_unweighted_loess = mae_unweighted_loess,
    mae_weighted_loess = mae_weighted_loess,
...
# Define updated grid search function
grid_search <- function(constructor_spans, driver_spans, 
                        constructor_weights, driver_weights, 
                        max_history_values) {
  
  results <- data.frame(constructor_span = numeric(0),
                        driver_span = numeric(0),
                        constructor_weight = numeric(0),
                        driver_weight = numeric(0),
                        max_history_value = numeric(0),
                        mae_unweighted = numeric(0),
                        mae_weighted = numeric(0),
                        mae_unweighted_loess = numeric(0),
                        mae_weighted_loess = numeric(0),
                        mae_unweighted_blended = numeric(0),
                        mae_weighted_blended = numeric(0))
  
  
  
  total_iterations <- length(constructor_spans) * length(driver_spans) * 
    length(constructor_weights) * length(driver_weights) * 
    length(max_history_values)
  
  iteration <- 1  # Track the iteration count
  
  for (constructor_span in constructor_spans) {
    for (driver_span in driver_spans) {
      for (constructor_weight in constructor_weights) {
        for (driver_weight in driver_weights) {
          for (max_history_value in max_history_values) {
            
            # Print progress update
            print(paste0("\n\nIteration ", iteration, " of ", total_iterations, 
                         ": constructor_span=", constructor_span, 
                         ", driver_span=", driver_span, 
                         ", constructor_weight=", constructor_weight, 
                         ", driver_weight=", driver_weight, 
                         ", max_history_value=", max_history_value,
                  "\n\n"))
            
            test_response <- decay_parameter_tune(
              constructor_span, driver_span, 
              constructor_weight, driver_weight,
              max_history_value
            )
            
            results <- results %>%
              add_row(constructor_span = constructor_span,
                      driver_span = driver_span,
                      constructor_weight = constructor_weight,
                      driver_weight = driver_weight,
                      max_history_value = max_history_value,
                      ...
                      mae_unweighted_loess = test_response$mae_unweighted_loess,
                      mae_weighted_loess = test_response$mae_weighted_loess,
                      mae_unweighted_blended = test_response$mae_unweighted_blended,
                      mae_weighted_blended = test_response$mae_weighted_blended)
            
            
            saveRDS(results, file = 'f1dataR - Exports/Data/loess_weight_grid_search_temp.rds')
...
constructor_spans <- c(0.3)
driver_spans <- c(0.3)
constructor_weights <- c(0.3, 0.5, 0.7)
driver_weights <- c(0.3, 0.5, 0.7)
max_history_values <- c(20, 40, 60)

# Run grid search
grid_results <- grid_search(constructor_spans, driver_spans, 
                            constructor_weights, driver_weights, 
                            max_history_values)


saveRDS(grid_results, file = 'f1dataR - Exports/Data/loess_weight_grid_search.rds')




readRDS(file = 'f1dataR - Exports/Data/loess_weight_grid_search_temp.rds')